...
public class SuffixTree {
    ...
    public List<Integer> search(String query) {
        ...
        if (nodes.size() > 0) {
            ...
            if (lastNode != null) {
                positions = getPositions(lastNode).stream().sorted().collect(Collectors.toList());
            }
        }
        return positions;
    }
    ...
    private void addChildNode(Node parentNode, String text, int position) {
        parentNode.getNextNodes().add(new Node(text, position));
    }
    ...
    private void splitNodeToParentAndChild(Node parentNode, String parentNewText, String childNewText) {
        ...
        if (parentNode.getNextNodes().size() > 0) {
            while (parentNode.getNextNodes().size() > 0) {
                childNode.getNextNodes().add(parentNode.getNextNodes().remove(0));
            }
        }
    ...
    private List<Node> getAllNodesInTraversePath(String pattern, Node startNode, boolean isAllowPartialMatch) {
        ...
        for (int i = 0; i < startNode.getNextNodes()
                .size(); i++) {
            Node currentNode = startNode.getNextNodes()
                    .get(i);
